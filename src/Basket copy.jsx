...
export default function Basket(props) {
  ...
  const { isPending, error, data, isFetching } = useQuery({
    QueryClient, // Add this line to pass the QueryClient instance
    queryKey: ["basket", basket],
    queryFn: () =>
      axios.get("https://order.basalam.com/basket?refresh=true").then((res) => {
        return res.data;
      }),
  });
    ...
    <div className="w-full flex items-center flex-col justify-center gap-10 my-10">
      {loading ? (
        ...
        <div className="w-full flex flex-col">
          ...
          {basket.vendors &&
            basket.vendors.map((vendor, index) => (
              <div key={index} className="flex flex-col gap-10 p-2 m-2 px-32">
                <h2 className="badge badge-primary badge-outline p-2">
                  غرفه ی {vendor.title}
                </h2>
                {vendor.items &&
